...
MULTI_DIR = os.path.join(BASE, 'contracts', 'multi_asset')
SRC_DIR = os.path.join(MULTI_DIR, 'src')

os.makedirs(SRC_DIR, exist_ok=True)

# ============================================================
# 1. Create multi_asset/Cargo.toml
# ============================================================
cargo_toml = '''[package]
...
stellar-tokens = { version = "0.7.1", features = ["testutils"] }
'''

with open(os.path.join(MULTI_DIR, 'Cargo.toml'), 'w') as f:
    f.write(cargo_toml)
print("Created multi_asset/Cargo.toml")

# ============================================================
# 2. Create multi_asset/src/lib.rs
# ============================================================
lib_rs = '''#![no_std]
use soroban_sdk::{contract, contractimpl, contracttype, contractevent, symbol_short, Address, Env, String, Vec, BytesN, Map, panic_with_error};
...
with open(os.path.join(SRC_DIR, 'lib.rs'), 'w') as f:
    f.write(lib_rs)
print(f"Created multi_asset/src/lib.rs ({len(lib_rs)} chars)")

# ============================================================
# 3. Update contracts/Cargo.toml to add multi_asset member
# ============================================================
workspace_path = os.path.join(BASE, 'contracts', 'Cargo.toml')
with open(workspace_path, 'r') as f:
    ws = f.read()

old_members = 'members = [".", "nft"]'
new_members = 'members = [".", "nft", "multi_asset"]'
if old_members in ws:
    ws = ws.replace(old_members, new_members)
    with open(workspace_path, 'w') as f:
        f.write(ws)
    print("Updated contracts/Cargo.toml members")
else:
    print("WARNING: Could not update workspace members")
