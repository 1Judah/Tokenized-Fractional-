...
COMP = os.path.join(FRONTEND, 'src', 'components')
PROFILE = os.path.join(COMP, 'ProfilePage')
...
# CSS already done above. Write JSX component
jsx = r'''import React, { useState, useEffect, useCallback } from 'react';
...
write_file(os.path.join(PROFILE, 'ProfilePage.jsx'), jsx)
print('ProfilePage.jsx written (' + str(len(jsx)) + ' chars)')

# Update App.jsx to add ProfilePage import and tab
app_path = os.path.join(FRONTEND, 'src', 'App.jsx')
with open(app_path, 'r', encoding='utf-8') as f:
    app = f.read()

# Add import for ProfilePage
if "import ProfilePage" not in app:
    old_import = "import TransactionHistory from './components/TransactionHistory/TransactionHistory';"
    new_import = old_import + "\nimport ProfilePage from './components/ProfilePage/ProfilePage';"
    app = app.replace(old_import, new_import)
    
    # Add profile tab in PATH_TO_VIEW
    old_path = "'/history': 'history'"
    new_path = "'/history': 'history', '/profile': 'profile'"
    app = app.replace(old_path, new_path)
    
    old_path2 = "history: '/history'"
    new_path2 = "history: '/history', profile: '/profile'"
    app = app.replace(old_path2, new_path2)
    
    # Add profile tab button after History tab
    old_history_tab = "History\n        </button>"
    new_history_tab = "History\n        </button>\n        <button\n          className={`${styles.tab} ${view === 'profile' ? styles.tabActive : ''}`}\n          onClick={() => setView('profile')}\n        >\n          Profile\n        </button>"
    app = app.replace(old_history_tab, new_history_tab)
    
    # Add ProfilePage route
    old_history_route = "view === 'history' ? (\n        <TransactionHistory />"
    new_history_route = "view === 'history' ? (\n        <TransactionHistory />\n      ) : view === 'profile' ? (\n        <ProfilePage />"
    app = app.replace(old_history_route, new_history_route)
    
    with open(app_path, 'w', encoding='utf-8') as f:
        f.write(app)
    print('App.jsx updated')
else:
    print('App.jsx already has ProfilePage')

print('\n=== ALL FILES CREATED/UPDATE ===')
